...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'OpenAI to NVIDIA NIM Proxy',
    reasoning_display: SHOW_REASONING, thinking_mode: ENABLE_THINKING_MODE });
});
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    const nimRequest = {
      model: nimModel, messages, temperature: temperature || 0.6,
    ...
    if (stream) {
      ...
      let buffer = '', reasoningStarted = false;
    ...
    res.status(error.response?.status || 500).json({
      error: { message: error.message || 'Internal server error',
        type: 'invalid_request_error', code: error.response?.status || 500 }
    });
  }
});
